...
我们将会看到，强如 Transformer 架构，细如 Attention 机制，也依然没有脱离 FNN 的底色。FNN 作为神经网络的基础在模型设计中随处可见，比如 FNN 加入循环逻辑就是 RNN，LSTM 解决 RNN 的长期依赖问题，而 GRU 简化 LSTM 的计算；FNN 引入卷积计算就是 CNN，而卷积在图像处理领域并不新鲜。_万变不离其宗，如果我们要解决新的问题，恐怕也要回到 FNN 的基础上求解和创新。_
